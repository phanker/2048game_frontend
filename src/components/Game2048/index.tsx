...
export default class Game2048 {
  ...
  constructor(options: Game2048Options) {
    ...
    if (this._options.state) {
      this._options.state.forEach((value, index) => {
        if (value > 0) {
          const newTile = new Game2048Tile(
            Math.floor(index / 4),
            index % 4,
            value,
            "2",
  ...
  generateTile(): boolean {
    ...
    const i = Math.floor(pos / 4),
      j = pos % 4;
    const newTile = new Game2048Tile(i, j, 0, "2", false);
    this._tiles.set(newTile.key, newTile);
  ...
  judge(): boolean {
    ...
    for (let i = 0; flag && i < 4; i++) {
      for (let j = 0; flag && j < 4; j++) {
        if (
          tilesArray[i * 4 + j] == null ||
          (i % 4 < 3 &&
            (tilesArray[(i + 1) * 4 + j] == null ||
              // @ts-ignore
              tilesArray[i * 4 + j].value ===
                tilesArray[(i + 1) * 4 + j]!.value)) ||
          (j % 4 < 3 &&
            (tilesArray[i * 4 + j + 1] == null ||
              // @ts-ignore
      ...
      diff.forEach((d) => {
        ...
        if (tile != null) {
          if (currentTile != null && tile.value === currentTile.value) {
            ...
            const targetPos = b + diff[targetPosIndex]; //0
  ...
  //todo.. 移植到move
  protected _getTopTiles(): {
    tilesArray: (Game2048Tile | null)[];
    toDelete: string[];
  } {
